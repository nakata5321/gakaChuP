...
class HashField:
    def __init__(self):
        self.__tagPopularity = pd.DataFrame(
            columns=["Hashtag", "Number of Posts", "Cost", "Potential Cost", "Score"]
        )
        self.__tagTree = {}
        self.__countTags = 0
    ...
    def addHashTag(self, hashtag):
        if self.checkInTree(hashtag):
            self.__tagPopularity.loc[int(self.__tagTree[hashtag]["id"])][1] = (
                int(self.__tagPopularity.loc[int(self.__tagTree[hashtag]["id"])][1]) + 1
            )
